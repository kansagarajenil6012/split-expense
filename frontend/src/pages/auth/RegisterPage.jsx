...
export default function RegisterPage() {
  ...
  const onPhoneSubmit = async (data) => {
    ...
    try {
      const formattedPhone = data.phone.startsWith('+') ? data.phone : `+91${data.phone}`; // Default to India
  ...
  return (
    <Box>
      <Box textAlign="center" mb={4}>
        ...
        <Typography color="text.secondary">Start splitting expenses with your groups</Typography>
      </Box>
      ...
      {tab === 0 ? (
        <Box component="form" onSubmit={handleEmailSubmit(onEmailSubmit)} noValidate>
          <TextField
            ...
            {...registerEmail('fullName')} error={!!emailErrors.fullName} helperText={emailErrors.fullName?.message}
            InputProps={{
              startAdornment: (
          ...
          >
            {isEmailSubmitting ? 'Creating...' : 'Create Account'}
          </Button>
        </Box>
      ) : (
        <Box>
          {!otpSent ? (
            ...
            <Box>
              ...
              <Button
                ...
                sx={{ mt: 3, mb: 2, py: 1.5, fontSize: '1rem' }}
              >
                {phoneLoading ? 'Verifying...' : 'Verify & Register'}
              </Button>
